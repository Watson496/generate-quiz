...
委譲機能を利用できる環境では、交差領域の確認、探索、生成、難易度の独立検査、解答露出検査、監査、最終出力を別々のagentへ割り当てる。親agentは起動toolが返した正規の識別子を起動直後に `execution.agents` へ記録し、各成果物に記載された担当識別子と照合する。候補変更時にも、継続して使う探索担当の識別子を別名へ置き換えない。利用できない環境では、その事実と理由を記録する。
