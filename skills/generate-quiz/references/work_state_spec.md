...
`final_input`は監査前に確定する。`final_input.quote_ids`には採用中の判断に用いた引用IDを過不足なく置く。`other_access_paths`の調査だけに用いた引用は含めない。監査合格後は、最終出力担当の割当記録を加える以外に、採用項目と`final_input`を変更しない。最終段階では、完成したMarkdownと監査に使った状態ファイルそのものを`work_state_check.py --stage final --output 完成稿.md 状態.json`へ渡し、該当する逐語引用の本文が出力に実在することを確認する。この文字列検査は引用と結論の意味上の対応や所在の正しさを判定しないため、外部資料を根拠にした判断に引用・所在・推論が対応していることは別途確認する。
