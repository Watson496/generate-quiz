...
この索引と各カタログは、元のファセットYAMLをスキルからの参照向けに変換したものです。分類内容・コード・階層は変更していません。あるノードを細分化するときは、該当する `FACET_NODE` ブロックの `DIRECT_CHILDREN` を全件候補にしてください。

## 参照方法

カタログ本体は1ファイルが最大約260KBあります。**Readで全文を読まず**、`scripts/facet_node.py` で必要な `FACET_NODE` ブロックだけを取り出してください。

```bash
python3 "$SKILL_DIR/scripts/facet_node.py" 'subject::7'      # 親ブロックとDIRECT_CHILDRENを表示
python3 "$SKILL_DIR/scripts/facet_node.py" --grep '音楽'      # ラベル部分一致でNODE_KEYを探す
```

1. 現在の親の `NODE_KEY`（`FACET::CODE` 形式）でブロックを取得する。
